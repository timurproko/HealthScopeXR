...
namespace Atomic.Events
{
    ...
    public partial class SceneEventBus : MonoBehaviour, IEventBus
    {
        ...
        public IReadOnlyCollection<int> DefinedEvents => _eventBus.DefinedEvents;
        ...
        public void Def(in int key) =>
            _eventBus.Def(in key);

        public void Def<T>(in int key) =>
            _eventBus.Def<T>(in key);

        public void Def<T1, T2>(in int key) =>
            _eventBus.Def<T1, T2>(in key);

        public void Def<T1, T2, T3>(in int key) =>
            _eventBus.Def<T1, T2, T3>(in key);
        ...
        public bool IsDefined(in int key) =>
            _eventBus.IsDefined(in key);

        public bool Undef(in int key) =>
            _eventBus.Undef(in key);
